...
consulta de apuntes, donativos discretos y anuncios. La web está publicada en
`https://valia2026.vercel.app/` y enlaza a las materias existentes en subcarpetas.
...
- `panel.html` ........... Página de panel del usuario (perfil, estadísticas y "mis apuntes"). Se abre con el engranaje del navbar.
...
- `_hub/panel.js` ........ Lógica de panel.html: auth-guard, bloqueo temporal de subidas, mis apuntes y borrar
- `_hub/updates.css` ..... Ajustes temporales de producto sin pisar las personalizaciones de `hub.css`
...
- Subidas de apuntes: **PAUSADAS** desde `config.js` → `funciones.permitirSubidas: false`. Ejecutar
  `SUPABASE-PAUSAR-SUBIDAS.sql` en Supabase para aplicar también el bloqueo del lado del servidor.
- Login: email/contraseña + **Google** (OAuth, ya implementado; falta activar proveedor en Supabase).
- Pagos/donativos: **PayPal** y **Mercado Pago**, dentro de un desplegable discreto en el footer.
...
- En el hub, el engranaje `#panelBtn` (visible al loguearse) navega a `panel.html`. Las nuevas subidas
  están deshabilitadas; el panel conserva la consulta y eliminación de apuntes existentes.
...
- [x] Publicada en Vercel: `https://valia2026.vercel.app/`.
- [ ] Verificar login real y el bloqueo de nuevas subidas una vez desplegada esta actualización.
...
- [ ] Verificar Sistema de Computación en ejecución y completar cualquier contenido pendiente.
