/* ============================================================
   ...
   Prácticas Profesionalizantes I · Operador Básico de Computadora
   ============================================================ */
(function () {
  "use strict";
  ...
  const TOOLS = [
    { id: "cotizador", icon: "money", title: "Cotizador de PC", desc: "Armá una computadora, elegí componentes y calculá el presupuesto total." },
    { id: "bytes", icon: "chip", title: "Conversor de almacenamiento", desc: "Convertí entre bits, bytes, KB, MB, GB y TB al instante." },
    { id: "diagnostico", icon: "tool", title: "Diagnóstico de fallas", desc: "Elegí un síntoma y descubrí la causa probable y la solución." },
    { id: "informe", icon: "doc", title: "Armador de informe técnico", desc: "Checklist con las etapas y partes de un informe técnico." },
  ];
  const toolById = id => TOOLS.find(t => t.id === id);

  const GAMES = [
    { id: "classify", icon: "target", title: "Clasificá el dispositivo", desc: "Mandá cada periférico a su tipo: entrada, salida, almacenamiento o conectividad." },
    { id: "vof", icon: "timer", title: "Verdadero o Falso", desc: "Decidí V o F sobre hardware, software y mantenimiento antes de que termine el tiempo." },
    { id: "memo", icon: "cards", title: "Memotest técnico", desc: "Emparejá cada término con su definición de memoria." },
    { id: "hangman", icon: "letters", title: "Ahorcado técnico", desc: "Adiviná la palabra del vocabulario con su pista." },
    { id: "connect", icon: "link", title: "Conectá", desc: "Uní cada concepto con su pareja en dos columnas." },
    { id: "spot", icon: "search", title: "Cazá el error", desc: "Decidí si la acción es una buena práctica o un error." },
    { id: "order", icon: "sort", title: "Ordená la secuencia", desc: "Ordená pasos y procesos: ciclos, etapas y procedimientos." },
  ];
  ...
  const ICONS = {
    ...
    doc: '<path d="M6 2h8l4 4v16H6z"/><path d="M14 2v4h4"/><path d="M9 13h6M9 17h6M9 9h2"/>',
  };
  function icon(name, cls) {
    return `<svg class="ic ${cls || ""}" viewBox="0 0 24 24" fill="none" stroke="currentColor"
      stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${ICONS[name] || ""}</svg>`;
  }
  // badge con el emoji de la unidad
  function badge(glyph, big) { return `<span class="ubadge${big ? " ubadge--lg" : ""}">${glyph}</span>`; }

  /* ---------------- Render seguro (sin LaTeX en PP1) ---------------- */
  window.MathJaxRender = function () { /* no-op: este sitio no usa fórmulas */ };

  /* ---------------- Progreso (localStorage) ---------------- */
  const STORE_KEY = "estudioPP1.progress.v1";
  ...
  function buildNav() {
    ...
    html += navItem("#/practica", icon("practice"), "Práctica (PSeInt y Proyecto)", null);
  ...
  function router() {
    ...
    else if (root === "tool") { crumb = toolById(id)?.title || "Herramienta"; renderToolPage(id); }
    ...
    else if (root === "practica" && id) { crumb = "Práctica"; renderPractice(id); }
    else if (root === "practica") { crumb = "Práctica"; renderPracticeMenu(); }
  ...
  function renderHome() {
    ...
    mount(`
      <h1 class="page-title">Prácticas Profesionalizantes I</h1>
      <p class="page-sub">Operador Básico de Computadora · Teoría, herramientas interactivas, práctica guiada, autoevaluación y juegos.</p>
      ...
      <div class="btn-row">
        <a class="btn btn--primary" href="#/practica">${icon("practice")} Práctica (PSeInt y Proyecto)</a>
    ...
    mount(`
      ...
      <div class="card center">
        ...
        <div class="btn-row" style="justify-content:center">
          ...
          ${tp ? `<a class="btn" href="#/practica/${tp.id}">${icon("practice")} Práctica</a>` : ""}
        </div>
      </div>
  ...
  function renderToolPage(id) {
    ...
    const c = mount(`<h1 class="page-title">${icon(t.icon, "ic--title")} ${t.title}</h1><p class="page-sub">${t.desc}</p><div id="toolMount"></div>
      <div class="pager"><span></span><a class="next" href="#/"><small>Inicio →</small>Volver</a></div>`);
  ...
  function gameBestText(id) {
    ...
    if (typeof b === "number") return `Mejor: ${b} ${id === "hangman" ? "palabras" : (id === "order" ? "secuencias" : "aciertos")}`;
  ...
  function renderGamesMenu() {
    ...
    mount(`
      ...
      <p class="page-sub">Aprendé jugando: clasificá dispositivos, corré contra el reloj y ejercitá la memoria. Tu mejor marca se guarda en este dispositivo.</p>
  ...
  function renderPracticeMenu() {
    ...
    mount(`
      <h1 class="page-title">${icon("practice","ic--title")} Práctica</h1>
      <p class="page-sub">Ejercicios de PSeInt resueltos paso a paso y la guía del Proyecto Final. Intentá resolverlos antes de revelar la solución.</p>
      <div class="unit-grid">${items}</div>`);
  }
  ...
  function renderPractice(id) {
    ...
    const c = mount(`
      ...
      <div class="pager"><span></span>
        <a class="next" href="#/practica"><small>Otra práctica →</small>Volver a la lista</a>
      </div>
    `);
  ...
  function buildSearchIndex() {
    ...
    [["Práctica (PSeInt y Proyecto)","Sección","#/practica"],
     ["Autoevaluación","Sección","#/quiz"],
     ["Flashcards","Sección","#/cards"],
     ["Juegos de aprendizaje","Sección","#/games"],
     ["Mi progreso","Sección","#/progress"]].forEach(([t,s,h]) =>
  ...
  /* ---------------- Tema ---------------- */
  function initTheme() {
    const saved = localStorage.getItem("estudioPP1.theme");
    ...
    tgl.addEventListener("click", () => {
      ...
      localStorage.setItem("estudioPP1.theme", t);
