...
(function () {
  ...
  const GAMES = [
    { id: "classify", icon: "target", title: "Clasificá el concepto", desc: "Mandá cada concepto a su subsistema: Procesos, Memoria, Archivos o E/S." },
    { id: "vof", icon: "timer", title: "Verdadero o Falso", desc: "Decidí V o F sobre afirmaciones de SO antes de que se acabe el tiempo." },
    { id: "memo", icon: "cards", title: "Memotest de SO", desc: "Emparejá términos con su significado, de memoria." },
    { id: "binconv", icon: "binary", title: "Conversión binaria", desc: "Convertí entre binario y decimal lo más rápido posible." },
    { id: "chain", icon: "sort", title: "Ordená la secuencia", desc: "Ordená estados, jerarquías y secuencias del SO." },
    { id: "hangman", icon: "letters", title: "Ahorcado de SO", desc: "Adiviná la palabra del vocabulario con su pista." },
    { id: "connect", icon: "link", title: "Conectá conceptos", desc: "Uní cada término con su definición en dos columnas." },
    { id: "sched", icon: "cpu", title: "¿Cuál se ejecuta?", desc: "Mirá la tabla de procesos y adiviná cuál corre primero." },
  ];
  ...
  const ICONS = {
    ...
    cpu: '<rect x="6" y="6" width="12" height="12" rx="2"/><path d="M9 3v3M15 3v3M9 18v3M15 18v3M3 9h3M3 15h3M18 9h3M18 15h3"/><rect x="9.5" y="9.5" width="5" height="5" rx="1"/>',
    memory: '<rect x="3" y="7" width="18" height="10" rx="2"/><path d="M7 17v3M11 17v3M15 17v3M8 11v2M12 11v2M16 11v2"/>',
    disk: '<circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="2.4"/><path d="M12 3v3"/>',
    bases: '<path d="M9 3 7 21M17 3l-2 18M4 8h16M3 16h16"/>',
    binary: '<path d="M7 4v7M5 4h2M5 11h4"/><rect x="13" y="4" width="5" height="7" rx="2.5"/><path d="M7 13v7M5 13h2M5 20h4"/><rect x="13" y="13" width="5" height="7" rx="2.5"/>',
  ...
  const UNIT_TOOL_ICON = { cpu: "cpu", memory: "memory", disk: "disk", bases: "bases", terminal: "terminal" };
  function badge(glyph, big) { return `<span class="ubadge${big ? " ubadge--lg" : ""}">${glyph}</span>`; }

  /* ---------------- Progreso (localStorage) ---------------- */
  const STORE_KEY = "estudioSO.progress.v1";
  ...
  function buildNav() {
    ...
    html += navItem("#/tool/cpu", icon("cpu"), "Planificador de CPU", null);
    html += navItem("#/tool/memory", icon("memory"), "Laboratorio de memoria", null);
    html += navItem("#/tool/disk", icon("disk"), "Planificación de disco", null);
    html += navItem("#/tool/bases", icon("bases"), "Conversor de bases", null);
    html += navItem("#/tool/terminal", icon("terminal"), "Terminal interactiva", null);
  ...
  function renderHome() {
    ...
    mount(`
      <h1 class="page-title">Plataforma de estudio de Sistemas Operativos</h1>
      <p class="page-sub">Teoría clara, simuladores interactivos, práctica guiada con los TP y autoevaluación. Todo en un solo lugar.</p>
      ...
      <div class="tool-grid">
        ${toolCard("#/tool/cpu","cpu","Planificador de CPU","Simulá FCFS, SJF, SRTF, Round Robin y Prioridades con diagrama de Gantt.")}
        ${toolCard("#/tool/memory","memory","Laboratorio de memoria","Traducí direcciones y simulá paginación, reemplazo y segmentación.")}
        ${toolCard("#/tool/disk","disk","Planificación de disco","Compará FCFS, SSTF, SCAN y C-SCAN con el recorrido del cabezal.")}
        ${toolCard("#/tool/bases","bases","Conversor de bases","Binario, octal, decimal y hexadecimal; potencias de 2 y bytes.")}
        ${toolCard("#/tool/terminal","terminal","Terminal interactiva","Practicá comandos de Linux en un shell simulado (ps, top, ls, kill…).")}
      </div>
  ...
  function renderToolPage(id) {
    const titles = {
      cpu: ["Planificador de CPU", "Cargá los procesos y compará los algoritmos de planificación con su diagrama de Gantt y métricas."],
      memory: ["Laboratorio de memoria", "Traducí direcciones (paginación y segmentación) y simulá el reemplazo de páginas."],
      disk: ["Planificación de disco", "Ingresá la cola de pistas y compará el recorrido y el desplazamiento del cabezal."],
      bases: ["Conversor de bases", "Convertí entre binario, octal, decimal y hexadecimal, con potencias de 2 y bytes."],
      terminal: ["Terminal interactiva", "Practicá comandos reales en un shell de Linux simulado, con archivos y procesos."],
    };
    const t = titles[id];
  ...
  function gameBestText(id) {
    ...
    if (typeof b === "number") return `Mejor: ${b} ${id === "hangman" ? "palabras" : id === "chain" ? "secuencias" : "aciertos"}`;
  ...
  function renderGamesMenu() {
    ...
    mount(`
      ...
      <p class="page-sub">Aprendé jugando: clasificá conceptos, corré contra el reloj y ejercitá la memoria. Tu mejor marca se guarda en este dispositivo.</p>
  ...
  function renderPracticeMenu() {
    ...
    mount(`
      ...
      <p class="page-sub">Ejercicios de cada TP con la solución paso a paso. Intentá resolverlos antes de revelar la respuesta.</p>
      <div class="unit-grid">${items}</div>`);
  }
  function renderPractice(id) {
    ...
    c.querySelectorAll(".exercise__toggle").forEach(b => b.addEventListener("click", () => {
      ...
      b.textContent = open ? "Ocultar solución" : "Ver solución"; b.classList.toggle("is-open", open);
    }));
  ...
  function renderQuiz(id) {
    ...
    function paint() {
      ...
      optsEl.addEventListener("click", e => {
        ...
        answered = true; const k = +b.dataset.k; const correct = q.a;
  ...
  function renderCardsMenu() {
    const items = UNITS.map(u => `
      <a class="unit-card" href="#/cards/${u.id}">
        ...
        <div class="unit-card__title">${u.title}</div>
        <div class="unit-card__desc">Repaso rápido de definiciones y conceptos.</div></a>`).join("");
    mount(`
  ...
  function renderProgress() {
    const rows = UNITS.map((u, i) => {
      const pct = unitPct(u.id); const q = progress.quiz[u.id];
      return `
        <div class="card" style="margin-bottom:14px">
          ...
          <div class="muted" style="font-size:12.5px;margin-top:8px">
            ${progress.read[u.id] ? "Teoría leída" : "Teoría pendiente"} · ${q ? `Quiz: ${q.best}/${q.total}` : "Quiz sin intentos"}
          </div>
        </div>`;
    }).join("");
    ...
    mount(`
      ...
      <div class="card center" style="margin-bottom:20px">
        <div class="quiz-score-ring">${globalPct()}%</div><p class="muted">Progreso general</p>
      </div>
      ${rows}
      <div class="card">
        ...
        <div class="game-best-grid">
          ${GAMES.map(g => `<div class="game-best">
            ...
            <div><div class="game-best__title">${g.title}</div><div class="muted" style="font-size:12.5px">${gameBestText(g.id)}</div></div>
          </div>`).join("")}
        </div>
      </div>
    ...
    $("#resetProg").addEventListener("click", () => {
      if (confirm("¿Seguro que querés borrar todo tu progreso?")) {
        progress = { read: {}, quiz: {} }; saveProgress(progress); router();
      }
    });
  }
  ...
  function buildSearchIndex() {
    ...
    UNITS.forEach((u, i) => {
      ...
      tmp.querySelectorAll("h2,h3").forEach(h => idx.push({ title: h.textContent.trim(), sub: u.title, href: `#/unit/${u.id}`, hay: h.textContent.toLowerCase() }));
    });
    ...
    [["Planificador de CPU","Herramienta","#/tool/cpu"],
     ["Laboratorio de memoria","Herramienta","#/tool/memory"],
     ["Planificación de disco","Herramienta","#/tool/disk"],
     ["Conversor de bases","Herramienta","#/tool/bases"],
     ["Terminal interactiva","Herramienta","#/tool/terminal"],
     ["Ejercicios de los TP","Práctica","#/practica"],
     ["Autoevaluación","Práctica","#/quiz"],
     ["Flashcards","Práctica","#/cards"],
     ["Juegos de aprendizaje","Sección","#/games"]].forEach(([t,s,h]) => idx.push({ title: t, sub: s, href: h, hay: t.toLowerCase() }));
    return idx;
  }
  let SEARCH_INDEX = [];
  function initSearch() {
  ...
  /* ---------------- Tema ---------------- */
  function initTheme() {
    const saved = localStorage.getItem("estudioSO.theme");
    ...
    tgl.addEventListener("click", () => {
      ...
      document.body.dataset.theme = t; localStorage.setItem("estudioSO.theme", t);
