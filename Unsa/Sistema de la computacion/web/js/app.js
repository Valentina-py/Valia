...
(function () {
  ...
  const TOOLS = [
    { id: "bases", icon: "binary", title: "Conversor de bases", desc: "Convertí entre binario, octal, decimal y hexadecimal, con parte fraccionaria." },
    { id: "enteros", icon: "sign", title: "Enteros con signo", desc: "Magnitud y signo, Ca1 y Ca2: rango, desbordamiento y sumas." },
    { id: "ieee754", icon: "float", title: "Punto flotante IEEE 754", desc: "Convertí decimal ↔ binario en simple precisión (32 bits)." },
    { id: "boole", icon: "truth", title: "Tablas de verdad", desc: "Tabla de cualquier función booleana, con FND, FNC y clasificación." },
    { id: "karnaugh", icon: "kmap", title: "Mapa de Karnaugh", desc: "Minimizá funciones booleanas de 2 a 4 variables." },
    { id: "codigos", icon: "code", title: "Códigos", desc: "BCD natural, Exceso-3, Aiken, Gray y ASCII." },
    { id: "cpu", icon: "cpu", title: "Planificador de CPU", desc: "FCFS, SJF, SRTF, Prioridad y Round Robin con diagrama de Gantt." },
    { id: "memoria", icon: "memory", title: "Memoria y paginación", desc: "Traducción de direcciones y reemplazo de páginas (FIFO, LRU, Óptimo)." },
    { id: "ensamblador", icon: "terminal", title: "Ensamblador 8086", desc: "Escribí y ejecutá programas del 8086 con sus registros." },
  ];
  const toolById = id => TOOLS.find(t => t.id === id);
  const UNIT_TOOL_ICON = { bases: "binary", enteros: "sign", ieee754: "float", boole: "truth", karnaugh: "kmap", codigos: "code", cpu: "cpu", memoria: "memory", ensamblador: "terminal" };

  const GAMES = [
    { id: "classify", icon: "target", title: "Clasificá el concepto", desc: "Mandá cada concepto a su unidad o área temática." },
    { id: "vof", icon: "timer", title: "Verdadero o Falso", desc: "Decidí V o F sobre los conceptos antes de que se acabe el tiempo." },
    { id: "memo", icon: "cards", title: "Memotest de siglas", desc: "Emparejá cada sigla con su significado: CPU, ALU, RAM…" },
    { id: "bits", icon: "binary", title: "Conversión binaria", desc: "Convertí entre decimal y binario contra el reloj." },
    { id: "gates", icon: "gate", title: "Compuertas lógicas", desc: "Dada la compuerta y sus entradas, elegí la salida correcta." },
    { id: "order", icon: "sort", title: "Ordená la secuencia", desc: "Ordená el ciclo de instrucción, la jerarquía de memoria y más." },
    { id: "hangman", icon: "letters", title: "Ahorcado técnico", desc: "Adiviná la palabra del vocabulario con su pista." },
    { id: "connect", icon: "link", title: "Conectá", desc: "Uní cada término con su definición en dos columnas." },
  ];
  ...
  const ICONS = {
    ...
    binary: '<path d="M7 4h2v7H7zM6 11h4M15 4h2v7h-2zM14 11h4M7 13h2v7H7zM6 20h4M15 13h2v7h-2zM14 20h4"/>',
    sign: '<path d="M5 12h6M14 9h6M14 15h6M17 9v6"/><path d="M8 9v6"/>',
    float: '<circle cx="6" cy="17" r="1.4" fill="currentColor" stroke="none"/><path d="M4 7h4M10 17h10M3 13l3-6 3 6"/>',
    kmap: '<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M3 15h18M9 3v18M15 3v18"/>',
    code: '<path d="m8 8-4 4 4 4M16 8l4 4-4 4M13 5l-2 14"/>',
    cpu: '<rect x="7" y="7" width="10" height="10" rx="1.5"/><rect x="9.5" y="9.5" width="5" height="5" rx="1"/><path d="M9 2v3M15 2v3M9 19v3M15 19v3M2 9h3M2 15h3M19 9h3M19 15h3"/>',
    memory: '<rect x="3" y="6" width="18" height="12" rx="2"/><path d="M7 6V3M12 6V3M17 6V3M7 18v3M12 18v3M17 18v3"/>',
    terminal: '<rect x="3" y="4" width="18" height="16" rx="2"/><path d="m7 9 3 3-3 3M13 15h4"/>',
    gate: '<path d="M5 6h5a6 6 0 0 1 0 12H5z"/><path d="M3 9h2M3 15h2M21 12h-3"/>',
    layers: '<path d="m12 3 9 5-9 5-9-5 9-5z"/><path d="m3 13 9 5 9-5M3 17l9 5 9-5"/>',
    cloud: '<path d="M7 18a4 4 0 1 1 .9-7.9A5 5 0 0 1 18 11a3.5 3.5 0 0 1-1 6.9z"/>',
    bolt: '<path d="M13 2 4 14h7l-1 8 9-12h-7z"/>',
    chip: '<rect x="6" y="6" width="12" height="12" rx="2"/><path d="M9 2v3M15 2v3M9 19v3M15 19v3M2 9h3M2 15h3M19 9h3M19 15h3"/>',
    free: '<path d="M12 3a9 9 0 1 0 9 9"/><path d="M21 3a9 9 0 0 0-9 9h9z"/>',
  };
  ...
  const STORE_KEY = "estudioSC.progress.v1";
  ...
  function renderHome() {
    ...
    mount(`
      <h1 class="page-title">Plataforma de estudio de Sistemas de Computación</h1>
      <p class="page-sub">Del bit al sistema operativo · Teoría, herramientas interactivas, práctica guiada y autoevaluación.</p>
  ...
  function renderToolPage(id) {
    const t = toolById(id);
    if (!t || !window.Tools || !window.Tools[id]) {
      return mount(`<h1 class="page-title">Herramienta</h1><div class="result-box empty">Esta herramienta todavía no está disponible.</div>`);
    }
  ...
  function renderGamesMenu() {
    ...
    mount(`
      ...
      <p class="page-sub">Aprendé jugando: clasificá conceptos, convertí binario y corré contra el reloj. Tu mejor marca se guarda en este dispositivo.</p>
  ...
  function renderPractice(id) {
    ...
    c.querySelectorAll(".exercise__toggle").forEach(b => {
      b.addEventListener("click", () => {
        ...
        if (open) window.MathJaxRender(sol);
      });
    });
    $("#toggleAll").addEventListener("click", () => {
      allShown = !allShown;
      c.querySelectorAll(".exercise__sol").forEach(s => { s.hidden = !allShown; if (allShown) window.MathJaxRender(s); });
      c.querySelectorAll(".exercise__toggle").forEach(b => {
  ...
  function renderCardsMenu() {
    const items = UNITS.map((u, i) => `
      <a class="unit-card" href="#/cards/${u.id}">
        ...
        <div class="unit-card__title">${u.title}</div>
        <div class="unit-card__desc">Repaso rápido de definiciones y siglas.</div></a>`).join("");
    mount(`
  ...
  /* ---------------- Tema ---------------- */
  function initTheme() {
    const saved = localStorage.getItem("estudioSC.theme");
    ...
    tgl.addEventListener("click", () => {
      ...
      localStorage.setItem("estudioSC.theme", t);
