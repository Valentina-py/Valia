const CACHE = "valia-programacion-2-v1";
const ASSETS = ["./", "index.html", "data.js", "../../_shared/course.css", "../../_shared/course.js", "../../../favicon.svg"];
self.addEventListener("install", event => event.waitUntil(caches.open(CACHE).then(cache => cache.addAll(ASSETS))));
self.addEventListener("activate", event => event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(key => key.startsWith("valia-programacion-2-") && key !== CACHE).map(key => caches.delete(key))))));
