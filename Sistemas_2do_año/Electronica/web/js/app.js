/* ============================================================
   ...
   Estudio Electrónica · Análisis de Circuitos (Boylestad)
   ============================================================ */
(function () {
  "use strict";
  ...
  const GAMES = [
    { id: "ohmrace", icon: "calc", title: "Resolvé con Ohm", desc: "Calculá V, I o R contra el reloj con la ley de Ohm." },
    { id: "vof", icon: "timer", title: "Verdadero o Falso", desc: "Decidí V o F sobre leyes y propiedades antes de que termine el tiempo." },
    { id: "classify", icon: "target", title: "Clasificá el componente", desc: "Mandá cada descripción a su componente: resistor, capacitor, inductor…" },
    { id: "colorcode", icon: "palette", title: "Código de colores", desc: "Mirá las bandas y elegí el valor de la resistencia." },
    { id: "memo", icon: "cards", title: "Memotest de unidades", desc: "Emparejá cada magnitud con su unidad de medida." },
    { id: "connect", icon: "link", title: "Conectá", desc: "Uní cada magnitud con su unidad en dos columnas." },
    { id: "hangman", icon: "letters", title: "Ahorcado eléctrico", desc: "Adiviná la palabra del vocabulario con su pista." },
    { id: "chain", icon: "sort", title: "Ordená resistencias", desc: "Ordená los valores de menor a mayor con sus prefijos." },
  ...
  const ICONS = {
    ...
    ohm: '<path d="M7 19h3v-2a6 6 0 1 1 4 0v2h3"/>',
    palette: '<path d="M12 3a9 9 0 1 0 0 18c1.5 0 2-1 2-2s-.5-1.5 0-2 1-.5 2-.5h1a3 3 0 0 0 3-3 8 8 0 0 0-8-8.5z"/><circle cx="7.5" cy="11" r="1.1" fill="currentColor" stroke="none"/><circle cx="12" cy="7.5" r="1.1" fill="currentColor" stroke="none"/><circle cx="16" cy="11" r="1.1" fill="currentColor" stroke="none"/>',
    resistor: '<path d="M2 12h3l2-5 3 10 3-10 3 10 2-5h2"/>',
    split: '<path d="M6 4v6a4 4 0 0 0 4 4h8M6 20v-6"/><path d="M15 11l3 3-3 3"/>',
  ...
  const TOOLS = [
    { id: "ohm", icon: "ohm", title: "Ley de Ohm", desc: "Ingresá dos valores (V, I, R o P) y obtené los otros dos." },
    { id: "colores", icon: "palette", title: "Código de colores", desc: "Elegí las bandas y leé el valor de la resistencia." },
    { id: "equivalente", icon: "resistor", title: "Resistencia equivalente", desc: "Combiná varias resistencias en serie o en paralelo." },
    { id: "divisor", icon: "split", title: "Divisor de tensión y corriente", desc: "Repartí voltaje y corriente entre dos resistencias." },
    { id: "reactancia", icon: "wave", title: "Reactancia e impedancia", desc: "Calculá X_L, X_C, impedancia y resonancia en CA." },
  ];
  ...
  const STORE_KEY = "estudioElec.progress.v1";
  ...
  function globalPct() { const sum = UNITS.reduce((s, u) => s + unitPct(u.id), 0); return Math.round(sum / UNITS.length); }
  ...
  function buildNav() {
    ...
    html += navItem("#/practica", icon("practice"), "Ejercicios resueltos", null);
  ...
  function renderHome() {
    ...
    mount(`
      <h1 class="page-title">Plataforma de estudio de Electrónica</h1>
      <p class="page-sub">Introducción al Análisis de Circuitos (Boylestad) · Teoría, herramientas interactivas, práctica guiada y autoevaluación.</p>
      ...
      <div class="btn-row">
        <a class="btn btn--primary" href="#/practica">${icon("practice")} Ejercicios resueltos</a>
  ...
  function renderGamesMenu() {
    ...
    mount(`
      ...
      <p class="page-sub">Aprendé jugando: resolvé con la ley de Ohm, leé códigos de colores y corré contra el reloj. Tu mejor marca se guarda en este dispositivo.</p>
  ...
  function renderPracticeMenu() {
    ...
    mount(`
      <h1 class="page-title">${icon("practice", "ic--title")} Ejercicios resueltos</h1>
      <p class="page-sub">Ejercicios típicos con la solución paso a paso. Intentá resolverlos antes de revelar la respuesta.</p>
      <div class="unit-grid">${items}</div>`);
  }
  function renderPractice(id) {
    ...
    const c = mount(`
      ...
      <div class="pager"><span></span><a class="next" href="#/practica"><small>Otros temas →</small>Volver a la lista</a></div>
    `);

    let allShown = false;
    c.querySelectorAll(".exercise__toggle").forEach(b => b.addEventListener("click", () => {
      const sol = c.querySelector(`#sol-${b.dataset.i}`); const open = sol.hidden; sol.hidden = !open;
  ...
  function renderQuiz(id) {
    ...
    function paint() {
      ...
      optsEl.addEventListener("click", e => {
        ...
        optsEl.querySelectorAll(".quiz-opt").forEach((el, idx) => { el.disabled = true; if (idx === correct) el.classList.add("correct"); else if (idx === k) el.classList.add("wrong"); });
    ...
    function paint() {
      ...
      const c = mount(`
        ...
        <div class="flashcard" id="fc"><div class="flashcard__inner">
          <div class="flashcard__face flashcard__face--front"><div class="flashcard__q">${card.q}</div><div class="flashcard__hint">Tocá para ver la respuesta</div></div>
          <div class="flashcard__face flashcard__face--back"><div class="flashcard__a">${card.a}</div><div class="flashcard__hint">Tocá para volver</div></div>
        </div></div>
    ...
    mount(`
      ...
      <div class="card">
        ...
        <div class="game-best-grid">
          ${GAMES.map(g => `<div class="game-best"><span class="game-best__ic">${icon(g.icon)}</span>
            <div><div class="game-best__title">${g.title}</div><div class="muted" style="font-size:12.5px">${gameBestText(g.id)}</div></div></div>`).join("")}
        </div>
      </div>
      <div class="btn-row"><button class="btn" id="resetProg">${icon("trash")} Reiniciar progreso</button></div>`);

    $("#resetProg").addEventListener("click", () => {
      if (confirm("¿Seguro que querés borrar todo tu progreso?")) { progress = { read: {}, quiz: {} }; saveProgress(progress); router(); }
    });
  }
  ...
  function buildSearchIndex() {
    ...
    [["Ejercicios resueltos", "Práctica", "#/practica"], ["Autoevaluación", "Práctica", "#/quiz"],
     ["Flashcards", "Práctica", "#/cards"], ["Juegos de aprendizaje", "Sección", "#/games"]]
  ...
  function initSearch() {
    ...
    input.addEventListener("input", () => {
      ...
      box.innerHTML = hits.length ? hits.map(h => `<div class="search__item" data-href="${h.href}">${h.title}<small>${h.sub}</small></div>`).join("") : `<div class="search__item">Sin resultados</div>`;
  ...
  /* ---------------- Tema ---------------- */
  function initTheme() {
    const saved = localStorage.getItem("estudioElec.theme");
    ...
    tgl.addEventListener("click", () => {
      ...
      document.body.dataset.theme = t; localStorage.setItem("estudioElec.theme", t);
